        ...
        <div className='Profilepic darkBg1'>
            <div className="changePic contered">
                ...
                <div style={{borderTop:'1px solid #8b8585d9' }}>
                    <button className='upload-btn' onClick={handelClick} style={{color:"#1EA1F7"}}>Upload Photo</button>
                    <input type="file" accept='image/*' onChange={(e)=>{setImage(e.target.files[0])}} ref = {hiddenFileInput} style={{display:"none"}}/>
                </div>
                <div style={{borderTop:'1px solid #8b8585d9' }}>
                    <button className='upload-btn' style={{color:'#ed4956'}}>Remove Current Photo</button>
                </div>
                <div style={{borderTop:'1px solid #8b8585d9' }}>
                    <button className='uplad-btn' onClick={changeprofile} style={{background:'none',border:'none',cursor:'pointer',fontSize:'15px'}}>Cancel</button>
                </div>
            </div>
        </div>
